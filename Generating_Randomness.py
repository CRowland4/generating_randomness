...
def create_triad_counts():
    ...
    triad_counts = {key: [0, 0] for key in triads}
...
class PredictionGame:
    counts = create_triad_counts()  # A dictionary of triads with ['0', '0'] as the starting values.

    def __init__(self):
        self.learning_string = ''
        self.test_string = ''
        self.triad_counts = dict(PredictionGame.counts)  # A copy of the class triad dictionary with no calculated values.
        self.prediction = ''
        self.capital = 1000
    ...
    def _setup(self):
        """Sets up the game and gets the initial string from which the game makes its predictions."""
        print("Please give the AI some data...")
    ...
    def _update_triad_counts(self):
        ...
        for i in range(len(self.learning_string) - 3):
            triad = self.learning_string[i: i + 3]
    ...
    def _update_test_string(self):
        """Updates the string from which predictions are made."""
    ...
    def _calculate_accuracy(self):
        ...
        print(f'Computer guessed {same} out of {same + dif} symbols correctly ({accuracy} %)')
        self.capital += dif
        self.capital -= same
